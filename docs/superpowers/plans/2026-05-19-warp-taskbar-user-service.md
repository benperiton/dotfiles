...
**Goal:** On work machines only, have chezmoi enable the `warp-taskbar.service` user unit (the Cloudflare WARP tray) shipped by `cloudflare-warp`.

**Architecture:** `git mv` the existing user-services script to a `.tmpl`, then append one `{{- if eq .machine_role "work" }}` block after the existing `ssh-agent`/`tmux` loop. The block is guarded by `rpm -q cloudflare-warp` (the WARP install is warn-and-continue, so the package may be absent) and `! systemctl --user is-enabled` (idempotent, matches the existing loop idiom); it runs `systemctl --user enable` then a tolerated `start`. Script `01` (the WARP install) and the waybar config are untouched.
...
- Do NOT use `enable --now` for warp-taskbar (the loop uses `--now` for the headless-safe ssh-agent/tmux; a tray started in a non-graphical apply context would fail under `set -euo pipefail`). The `enable` + tolerated `start ... ||` form is deliberate.
...
grep -q 'systemctl --user is-enabled warp-taskbar.service' /tmp/svc-work-render.sh && echo "is-enabled guard OK"
grep -q 'systemctl --user enable warp-taskbar.service' /tmp/svc-work-render.sh && echo "enable OK"
...
Expected: all six lines print (`rpm guard OK`, `is-enabled guard OK`, `enable OK`, `tolerated start OK`, `existing loop intact OK`, `daemon-reload intact OK`).
...
git commit -m "user-services: enable warp-taskbar tray on work machines"
git rev-parse HEAD
...
- **Spec coverage:** rename to `.tmpl` (Step 2); work-gated block appended after the loop with `rpm -q` + `is-enabled` guards, `enable` + tolerated `start` (Step 3); script 01 / waybar untouched (only file 09 in Steps 7-8); personal renders nothing and the existing loop is preserved (Step 6, including byte-identical assertion); verification via work render asserts + `bash -n` + personal render + `chezmoi diff` + no dashes (Steps 4-7 and the Step 3 no-dash note). Every spec section maps to a step.
- **Placeholder scan:** none. No TBD/TODO; all code shown in full; commands have expected output.
- **Type/name consistency:** `warp-taskbar.service`, `cloudflare-warp`, `run_once_after_09-setup-user-services.sh.tmpl` are spelled identically across Step 3's block and Steps 4/6/7/8 assertions and commands. The Step 6 `diff` heredoc exactly reproduces the current script body verified in this session (identity-render confirmed).
